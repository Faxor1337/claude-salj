...
app.get('/api/analytics', requireAuth, async (req, res) => {
    try {
        ...
        const provpaket = await pool.query(`
            SELECT
                COALESCE(SUM(CASE WHEN type='in' THEN quantity ELSE 0 END), 0) as total_sent,
                COALESCE(SUM(CASE WHEN type='out' THEN quantity ELSE 0 END), 0) as total_sold,
                COALESCE(SUM(CASE WHEN type='in' THEN quantity ELSE -quantity END), 0) as total_remaining
            FROM provpaket
        `);
